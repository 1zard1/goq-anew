...
GMP Модель
GMP модель состоит из трёх основных компонентов:

Goroutines (G): Лёгкие потоки, которые выполняют функции.

Machine (M): Операционные системные потоки, которые выполняют goroutines.

Processor (P): Логические процессоры, которые управляют выполнением goroutines на M.

Локальные и Глобальные Очереди
Локальные Очереди: Каждое P имеет свою локальную очередь goroutines. Это позволяет уменьшить конкуренцию между потоками.

Глобальная Очередь: Если локальная очередь P пуста, P может брать goroutines из глобальной очереди.

Кражи Горутин и Перераспределение Нагрузки
Кражи Горутин: Если P не имеет goroutines для выполнения, оно может "украсть" goroutines из локальной очереди другого P. Это помогает сбалансировать нагрузку между процессорами.

Перераспределение Нагрузки: Периодически планировщик перераспределяет goroutines между P, чтобы обеспечить равномерное распределение нагрузки.

Кооперативная и Конкурентная Модели
Кооперативная Модель: В этой модели goroutines добровольно уступают управление, что может привести к проблемам, если goroutine не уступает управление.

Конкурентная Модель: В этой модели планировщик может принудительно прерывать выполнение goroutines, чтобы обеспечить более равномерное распределение времени выполнения.
